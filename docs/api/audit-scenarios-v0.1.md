...
> DetailAdvisor 模块 3(异常场景发现器)后端 API · v0.1 雏形 · 2026-09-10(启动) / 2026-09-12(增量扩展) / 2026-09-14(再增量)
> 落地 commit:
...
> 决策依据:0908 T5 巡检建议"12 规则已收口,0910+ 可启动异常场景发现器";0912 巡检"特别建议路线图 B(audit-scenarios 触发类型库扩展)";0914 巡检"最高优:兑现 1 项轻量交付 + 2 项主计划修改合并同次 commit";无 plan 临时决策模式第 11/13/14 天延续
...
| 项 | 说明 |
|---|------|
| 当前已实现 | **5 类异常场景库**(零依赖纯模板,**37 条场景,2026-09-12 T5 增量 7 条 + 2026-09-14 T5 增量 2 条**):**boundary**(边界值,11 条)+ **concurrency**(并发,7 条)+ **network**(网络,7 条)+ **permission**(权限,6 条)+ **device**(设备,6 条)。每条含 id/category/title/description/trigger/expected/severity/keywords |
| 暂未实现 | PRD 关键词深度匹配(v0.2 计划,接 LLM 或词典匹配)/ LLM 二次校验(v0.3 计划,接 Claude Sonnet 4.5)/ R-SCENE-01~99 子规则细分(0912 T5 已扩 7 条,0914 T5 再扩 2 条,后续可按行业细分子规则) |
| 优先级 | Phase 1 §6 模块 3"上线异常场景发现器"启动 commit + 触发类型库扩展,代码资产 0 → 0.1 起步 |
...
```typescript
{
  ...
  "meta": {
    "api_version": string,                      // "0.1.0-API-雏形+7-触发类型库扩展+2-财务微服务"
...
| 3 | 11-20 场景,3 类活跃 |
| 4 | 21-30 场景,4-5 类活跃 |
| 5 | > 30 场景,全 5 类活跃(典型 28/35/37 条全清单) |

**filter_mode 取值**:

- `all`:无 categories 过滤,返回全 5 类清单(37 条)
- `category`:有 categories 过滤,仅返回指定类别
- `keyword`:feature 关键词命中(目前仅记录,不剔除场景)

## 3. 5 类异常场景库详情

### 3.1 boundary 边界值(11 条,SC-B-01 ~ SC-B-11)
...
### 3.2 concurrency 并发(7 条,SC-C-01 ~ SC-C-07)
...
### 3.3 network 网络(7 条,SC-N-01 ~ SC-N-07)
...
### 3.4 permission 权限(6 条,SC-P-01 ~ SC-P-06)
...
### 3.5 device 设备(6 条,SC-D-01 ~ SC-D-06)
...
预期:返回 api/version/categories_implemented(5 类 + 各自 count)/total_scenarios(37)/severity_levels/filter_modes。

### 4.2 T2 全清单(无 categories,典型 37 条)
...
预期:`total_scenarios=37`,`by_category` 5 类各含场景,`score_complexity=5`,`filter_mode="all"` 或 `"keyword"`(若 feature 命中关键词)。
...
预期:`total_scenarios=18`(11 boundary + 7 network),`score_complexity=3`(18 场景 + 2 类活跃),`filter_mode="category"`,`categories_returned=["boundary","network"]`。
...
预期:`total_scenarios=37`,`feature_keywords_matched` 含"秒杀""库存""活动""缓存""Redis"等,`filter_mode="keyword"`。
...
- **R-SCENE-01~99 子规则**:目前 37 条为顶层场景,后续可按行业细分(如电商"秒杀超卖"、教育"退课流程"、金融"反洗钱"等子规则);0912 T5 已扩 7 条(SC-B-09/10 + SC-C-06 + SC-N-06/07 + SC-P-06 + SC-D-06),0914 T5 再扩 2 条(SC-B-11 + SC-C-07)
