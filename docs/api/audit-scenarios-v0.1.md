...
> DetailAdvisor 模块 3(异常场景发现器)后端 API · v0.1 雏形 · 2026-09-10(启动) / 2026-09-12(增量扩展)
> 落地 commit:
...
> 决策依据:0908 T5 巡检建议"12 规则已收口,0910+ 可启动异常场景发现器";0912 巡检"特别建议路线图 B(audit-scenarios 触发类型库扩展)";无 plan 临时决策模式第 11/13 天延续
...
| 项 | 说明 |
|---|------|
| 当前已实现 | **5 类异常场景库**(零依赖纯模板,**35 条场景,2026-09-12 T5 增量 7 条**):**boundary**(边界值,10 条)+ **concurrency**(并发,6 条)+ **network**(网络,7 条)+ **permission**(权限,6 条)+ **device**(设备,6 条)。每条含 id/category/title/description/trigger/expected/severity/keywords |
| 暂未实现 | PRD 关键词深度匹配(v0.2 计划,接 LLM 或词典匹配)/ LLM 二次校验(v0.3 计划,接 Claude Sonnet 4.5)/ R-SCENE-01~99 子规则细分(0912 T5 已扩 7 条,后续可按行业细分子规则) |
...
```typescript
{
  ...
  "meta": {
    "api_version": string,                      // "0.1.0-API-雏形+7-触发类型库扩展"
...
| 3 | 11-20 场景,3 类活跃 |
| 4 | 21-30 场景,4-5 类活跃 |
| 5 | > 30 场景,全 5 类活跃(典型 28/35 条全清单) |

**filter_mode 取值**:

- `all`:无 categories 过滤,返回全 5 类清单(35 条)
- `category`:有 categories 过滤,仅返回指定类别
- `keyword`:feature 关键词命中(目前仅记录,不剔除场景)

## 3. 5 类异常场景库详情

### 3.1 boundary 边界值(10 条,SC-B-01 ~ SC-B-10)
...
### 3.2 concurrency 并发(6 条,SC-C-01 ~ SC-C-06)
...
### 4.2 T2 全清单(无 categories,典型 35 条)
...
预期:`total_scenarios=35`,`by_category` 5 类各含场景,`score_complexity=5`,`filter_mode="all"` 或 `"keyword"`(若 feature 命中关键词)。
...
预期:`total_scenarios=17`(10 boundary + 7 network),`score_complexity=3`(17 场景 + 2 类活跃),`filter_mode="category"`,`categories_returned=["boundary","network"]`。
...
预期:`total_scenarios=35`,`feature_keywords_matched` 含"秒杀""库存""活动""缓存""Redis"等,`filter_mode="keyword"`。
...
- **R-SCENE-01~99 子规则**:目前 35 条为顶层场景,后续可按行业细分(如电商"秒杀超卖"、教育"退课流程"、金融"反洗钱"等子规则);0912 T5 已扩 7 条(SC-B-09/10 + SC-C-06 + SC-N-06/07 + SC-P-06 + SC-D-06)
